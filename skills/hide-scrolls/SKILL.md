...
Because a directory literally named `scrolls` (no dot) is a more generic name than `.scrolls`, false positives are more plausible here than in `/unhide-scrolls` — the `STARTER.md`-presence guard described below is what keeps recursion safe regardless.
