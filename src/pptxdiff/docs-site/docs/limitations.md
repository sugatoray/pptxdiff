...
| **Standalone binaries are unsigned/ad-hoc-signed, and not yet on GitHub Releases** | No code-signing certificate exists for this project — Windows SmartScreen and macOS Gatekeeper both warn on a freshly-downloaded copy. Only available as CI workflow artifacts today, not attached to a tagged release. macOS specifically can't be cross-compiled from Linux/Windows for this reason (an unsigned binary may not even launch on Apple Silicon) — it builds on a real `macos-latest` runner instead. See [Getting Started → Option E](getting-started.md#option-e-standalone-binary-no-nodejs-at-all). |
