...
**No Node.js at all?** A standalone native executable per OS+chip runs this exact, unmodified `bin/cli.js` with the Node runtime embedded inside it — see [Getting Started → Option E](getting-started.md#option-e-standalone-binary-no-nodejs-at-all).
