...
- **Not done**: enabling "GitHub Actions" as the Pages source in the repo's Settings → Pages UI — that's a one-time manual step outside this repo's files (workflow permissions alone don't do it), the first push after merging this needs it. `site_url` in `mkdocs.yml` (already `https://sugatoray.github.io/pptxdiff/`) doesn't need to change.
